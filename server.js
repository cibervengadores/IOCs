...
const SSH_PRIVATE_KEY = process.env.SSH_PRIVATE_KEY; // Carga la clave privada
const FILE_PATH = 'peticiones.md';

// Configura la clave SSH antes de cualquier operación
const setSSHKey = () => {
  const sshDir = './.ssh'; // Crea un directorio .ssh en tu directorio actual
  if (!fs.existsSync(sshDir)) {
    fs.mkdirSync(sshDir); // Crea el directorio si no existe
  }

  fs.writeFileSync(`${sshDir}/id_rsa`, SSH_PRIVATE_KEY);
  fs.chmodSync(`${sshDir}/id_rsa`, 0o600); // Establece permisos correctos
};
...
const addToFile = async (petition) => {
  try {
    ...
    // Configura la URL del repositorio usando SSH
    const gitUrl = `git@github.com:${GITHUB_USER}/${GITHUB_REPO}.git`;
    ...
    if (error.message.includes('rejected')) {
      ...
      try {
        ...
        await git.push(gitUrl, 'main'); // Luego intenta hacer push forzado
      } catch (pullError) {
